...
public class ACMECollegeService implements Serializable {
    ...
    private static final Logger LOG = LogManager.getLogger( ACMECollegeService.class.getName() );
    ...
    public void buildUserForNewStudent(Student newStudent) {
        ...
        SecurityRole userRole = em.createNamedQuery(SecurityRole.FIND_BY_ROLE_NAME, SecurityRole.class)
                ...
                .getSingleResult(); /* TODO ACMECS01 - Use NamedQuery on SecurityRole to find USER_ROLE */ 
    ...
    public void deleteStudentById(int id) {
        ...
        if (student != null) {
            em.refresh(student);
           // TypedQuery<SecurityUser> findUser = em.createQuery( SecurityUser.FIND_USER_BY_ID , SecurityUser.class);
...
//            		null;
    ...
    @Transactional
    public <T> T deleteById(Class<T> entity, String namedQuery, int id) {
    	T entityToDelete =  getById( entity,  namedQuery,  id);
    ...
    public StudentClub deleteStudentClub(int id) {
        ...
        if (sc != null) {
            ...
            list.forEach(m -> {
                if (m.getCard() != null) {
                    MembershipCard mc = getById(MembershipCard.class, FIND_BY_ID, m.getCard().getId());
    ...
    public void deleteProfessorId(int id) {
        Professor professor = getById(Professor.class, "Professor.findById", id);
        if (professor != null) {
            em.refresh(professor);

            Query query = em.createNamedQuery("CourseRegistration.updateProfessorIdToNull");
